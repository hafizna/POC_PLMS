...
import { ArrowRight, Plus, RotateCcw, Trash2, X } from "lucide-react";
import type {
  Bay,
  Busbar,
  LineRelation,
...
import { useProsetStore, type CtVtOverride, type CtVtOverrideInput, type MiniNmmOverride } from "../../store/useProsetStore";
import {
  LCD_DIST_REGISTRY,
...
type Props = {
  ...
  override?: MiniNmmOverride;
};

export function MiniNmmEditor({ caseId, miniNmm, override }: Props) {
  const addRelation = useProsetStore((s) => s.addMiniNmmRelation);
  const addSubstation = useProsetStore((s) => s.addMiniNmmSubstation);
  const addBusbar = useProsetStore((s) => s.addMiniNmmBusbar);
  const addBay = useProsetStore((s) => s.addMiniNmmBay);
  const addTerminal = useProsetStore((s) => s.addMiniNmmTerminal);
  const addIed = useProsetStore((s) => s.addMiniNmmIed);
  ...
  const removeEntry = useProsetStore((s) => s.removeMiniNmmEntry);
  const resetOverrides = useProsetStore((s) => s.resetMiniNmmOverrides);
  ...
  const computeMatchCount = (newRelationId: string | null): number => {
    ...
    const nodes = networkNodesFromMiniNmm(miniNmm);
    const lines = networkLinesFromMiniNmm(miniNmm);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-slate-100">
        <AddSubstationForm
          ...
          onSubmit={(substation, busbar) => {
            addSubstation(caseId, substation);
            addBusbar(caseId, busbar);
            setRecent({
              kind: "substation",
        ...
        <AddRelationForm
          ...
          onSubmit={(relation, bays, busbars, terminals) => {
            for (const busbar of busbars) addBusbar(caseId, busbar);
            for (const bay of bays) addBay(caseId, bay);
            for (const terminal of terminals) addTerminal(caseId, terminal);
            addRelation(caseId, relation);
        ...
        <AddIedForm
          ...
          onSubmit={(ied) => {
            ...
            const owningRelation = miniNmm.lineRelations.find(
          ...
          <div className="space-y-1">
            {override?.substations?.map((s) => (
              <OverrideRow
                ...
                onRemove={() => removeEntry(caseId, "substation", s.id)}
              />
            ))}
            ...
            {override?.relations?.map((r) => (
              <OverrideRow
                ...
                onRemove={() => removeEntry(caseId, "relation", r.id)}
              />
            ))}
            {override?.bays?.map((b) => (
              <OverrideRow
...
function AddSubstationForm({
  ...
  const submit = () => {
    ...
    if (miniNmm.substations.some((s) => s.normalizedName === normalizedName)) {
      return setError("Substation dengan nama ini sudah ada di mini-NMM.");
    }
...
function AddRelationForm({
  ...
  const [fromId, setFromId] = useState(miniNmm.substations[0]?.id ?? "");
  const [toId, setToId] = useState(miniNmm.substations[1]?.id ?? "");
  ...
  const submit = () => {
    ...
    const fromSub = miniNmm.substations.find((s) => s.id === fromId);
    const toSub = miniNmm.substations.find((s) => s.id === toId);
    if (!fromSub || !toSub) {
    ...
    if (miniNmm.lineRelations.some((r) => r.id === id)) {
    ...
    const fromBusbar =
      miniNmm.busbars.find((b) => b.substationId === fromId) ??
      newBusbars[
        newBusbars.push({
    ...
    const toBusbar =
      miniNmm.busbars.find((b) => b.substationId === toId) ??
      newBusbars[
        newBusbars.push({
    ...
    <div className="px-4 py-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <SelectField
          ...
          value={fromId}
          onChange={setFromId}
          options={miniNmm.substations.map((s) => ({ value: s.id, label: `${s.shortCode} ${s.name}` }))}
        />
        <SelectField
          label="To substation"
          value={toId}
...
function AddIedForm({
  ...
  const [bayId, setBayId] = useState(miniNmm.bays[0]?.id ?? "");
  ...
  const submit = () => {
    ...
    if (miniNmm.relayIeds.some((i) => i.bayId === bayId)) {
    ...
    <div className="px-4 py-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <SelectField
          ...
          value={bayId}
          onChange={setBayId}
          options={miniNmm.bays.map((b) => {
            const sub = miniNmm.substations.find((s) => s.id === b.substationId);
            return {
              value: b.id,
...
function CtVtMasterForm({
  ...
  const [iedId, setIedId] = useState(miniNmm.relayIeds[0]?.id ?? "");
  ...
  const selectedIed = miniNmm.relayIeds.find((ied) => ied.id === iedId);
  const selectedBay = selectedIed ? miniNmm.bays.find((bay) => bay.id === selectedIed.bayId) : undefined;
  const selectedSub = selectedBay ? miniNmm.substations.find((sub) => sub.id === selectedBay.substationId) : undefined;
  const effective = getEffectiveCtVt(selectedIed, overrides);

  useEffect(() => {
    const nextIed = miniNmm.relayIeds.find((ied) => ied.id === iedId) ?? miniNmm.relayIeds[0];
    if (!nextIed) return;
    ...
    setMessage(null);
  }, [iedId, miniNmm.relayIeds, overrides]);
    ...
    <div className="px-4 py-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <SelectField
          ...
          value={iedId}
          onChange={setIedId}
          options={miniNmm.relayIeds.map((ied) => {
            const bay = miniNmm.bays.find((item) => item.id === ied.bayId);
            const sub = bay ? miniNmm.substations.find((item) => item.id === bay.substationId) : undefined;
            return {
              value: ied.id,
