  ...
  return (
    <div className="space-y-5">
      <header className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
        <div className="px-6 py-5 bg-gradient-to-r from-brand-ink via-slate-900 to-brand-ink-2 text-white">
      ...
      >
        <div className="flex items-start gap-2">
          {faultContext.status === "ready" ? (
            <CheckCircle2 className="h-4 w-4 text-brand-accent-dark mt-0.5" />
          ) : (
      ...
      <div className="grid items-start gap-5 xl:grid-cols-[minmax(360px,0.82fr)_minmax(0,1.45fr)]">
        ...
        <section className="space-y-4">
          <div className="flex flex-col gap-3 rounded-2xl border border-brand-accent/40 bg-brand-accent/10 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="text-xs font-semibold uppercase tracking-[0.14em] text-brand-accent-dark">
                Lanjut ke MVP 1B
              </div>
            ...
            <button
              ...
              className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg bg-brand-ink px-4 py-2 text-xs font-semibold text-white shadow-sm hover:bg-black"
            >
              Bandingkan actual
            ...
            {showTrace && (
              <div className="border-t border-slate-200 px-5 py-1">
                {result.trace.map((step, index) => (
                  ...
                  >
                    <span className="flex h-6 w-6 items-center justify-center rounded-full bg-brand-accent/10 text-[11px] font-bold text-brand-accent-dark">
                      {index + 1}
                    </span>
                    <div>
      ...
      <div className="rounded-xl border border-brand-accent/40 bg-brand-accent/10 px-4 py-3 text-xs leading-5 text-brand-accent-dark">
  ...
  return (
    <button
      ...
      className={`group rounded-2xl border p-4 text-left transition-all ${
        active
          ? "border-brand-accent bg-brand-accent/10 shadow-sm ring-2 ring-brand-accent/20"
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        {active && <CheckCircle2 className="h-5 w-5 text-brand-accent-dark" />}
      </div>
      <div className="mt-3 text-sm font-semibold text-slate-900">{title}</div>
      <div className="mt-0.5 text-xs font-medium text-brand-accent-dark">{subtitle}</div>
    ...
    <label className="block">
      ...
      <div
        className={`flex overflow-hidden rounded-lg border ${
          derived ? "border-slate-200 bg-slate-100" : "border-brand-accent/40 bg-white"
        }`}
      >
        <input
          type="number"
          ...
          className="min-w-0 flex-1 bg-transparent px-3 py-2 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-inset focus:ring-brand-accent/30"
        />
        {unit && (
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="w-full rounded-lg border border-brand-accent/40 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-brand-accent focus:ring-2 focus:ring-brand-accent/20"
      />
    </label>
  );
}
    ...
    <label className="flex cursor-pointer items-center justify-between rounded-lg border border-slate-200 px-3 py-2.5">
      ...
      <button
        ...
        className={`relative h-5 w-9 rounded-full transition-colors ${
          checked ? "bg-brand-ink" : "bg-slate-300"
        }`}
      >
        <span
...
function LineSelect({
  ...
  return (
    <div className="rounded-xl border border-slate-200 p-3">
      <div className="mb-2 flex items-center gap-2">
        <span className="rounded-md bg-brand-ink px-1.5 py-0.5 text-[10px] font-bold text-brand-accent">
          {slot}
        </span>
      ...
      <select
        value={value}
        onChange={(event) => onChange(event.target.value)}
        className="w-full rounded-lg border border-brand-accent/40 bg-white px-2.5 py-2 text-xs text-slate-800 outline-none focus:border-brand-accent"
      >
