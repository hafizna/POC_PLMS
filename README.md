...
> **Boundary data saat ini — SSOT-2B:** Asset & Setting Explorer tetap merupakan projection canonical read-only, tetapi sekarang menyediakan jalur governed `Usulkan perubahan`. Stable line/bay scope dan alasan perubahan dibawa ke Setting Case; proposal menyimpan canonical target, baseline revision, field-level diff/evidence, dan activation policy. Active data tidak diedit. Persistence masih Zustand/localStorage POC; repository boundary dan backend database adalah SSOT-2C–2D.
...
|---|---|---|
| **F1 — Foundation** | Canonical entity identity, effective dating, `Setting Case`/`Setting Package`/`Setting Revision` schema, lifecycle state machines, org scope/roles. | **SSOT-2B UI slice implemented.** SSOT-1 menyediakan read-only Asset 360; SSOT-2A menyediakan executable authority/revision contract. Explorer kini memulai governed proposal dengan stable scope, case wizard ter-prefill, dan case editor menyimpan canonical target, baseline revision, field diff/evidence, serta activation policy. Repository/backend, locking, organizational RBAC, submit/approval UI, dan atomic database activation masih belum. |
...
1. **SSOT-2C** — repository boundary agar UI/domain tidak tergantung langsung pada Zustand/localStorage.
2. **SSOT-2D** — implement backend schema, migration, atomic activation, concurrency control, dan RBAC setelah contract UI/repository stabil.
3. **F1 workflow governance** — submit/review/approval/activation UI di atas revision authority yang sama.
4. **E1 workflow** — lanjutkan engineering review, canonical Setting Revision, dan issuance; ekspansi trafo/OCR-GFR/multi-vendor tetap ditahan.
