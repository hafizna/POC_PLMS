...
> **Cross-agent handoff:** konteks implementasi terkini, acceptance flow, known limitations, dan urutan future development dirangkum di [`DEVELOPMENT_HANDOFF.md`](./DEVELOPMENT_HANDOFF.md). Dokumen ini wajib dibaca sebelum pekerjaan dilanjutkan oleh agent lain.
...
> **Boundary data saat ini — SSOT-2C:** Asset & Setting Explorer tetap merupakan projection canonical read-only dan SSOT-2B menyediakan jalur governed `Usulkan perubahan`. Repository ports serta local snapshot adapter sekarang memisahkan kontrak domain/persistence dari Zustand storage. Browser localStorage masih adapter POC; belum ada backend database, server-side RBAC, atau atomic activation transaction.
...
|---|---|---|
| **F1 — Foundation** | Canonical entity identity, effective dating, `Setting Case`/`Setting Package`/`Setting Revision` schema, lifecycle state machines, org scope/roles. | **SSOT-2C implemented for the POC.** SSOT-1 read-only Asset 360, SSOT-2A authority/revision contract, dan SSOT-2B governed proposal UI sudah terhubung. Repository ports, persisted snapshot allowlist, local-storage adapter, memory test adapter, dan optimistic-concurrency reference tersedia. Backend, organizational RBAC, submit/approval UI, dan atomic database activation masih belum. |
...
1. **SSOT-2D.0** — review authority/business flow, lalu ADR/ERD, transaction boundary, storage split, migration, dan rollback design; belum provisioning production DB.
2. **SSOT-2D.1** — local/dev database adapter dan dual contract test terhadap adapter POC.
3. **SSOT-2D.2** — staging multi-user ketika submit/review/approval mulai lintas user; tambah identity, UPT/UIT RBAC, locking, dan atomic activation.
4. **E1 workflow** — lanjutkan review, canonical Setting Revision, dan issuance setelah persistence authority kredibel; ekspansi trafo/OCR-GFR/multi-vendor tetap ditahan.
...
- **SSOT-2B governed proposal UI:** Asset 360 menyediakan `Usulkan perubahan` berbasis alasan; stable line/bay scope diteruskan ke case wizard tanpa pemilihan ulang, case baru langsung dibuka, dan Proposed Data Revision dibungkus sebagai Data Change Proposal dengan canonical target, baseline binding, only-changed field diff, evidence, serta activation policy. Active master tetap read-only.
- **SSOT-2C repository boundary:** persistence-neutral ports mendefinisikan Setting Case, governed revision/proposal/activation, source observation, dan audit. Snapshot POC memakai allowlist eksplisit serta local/memory adapter; transient UI state tidak ikut disimpan dan stale-write behavior memiliki optimistic-concurrency reference regression.
...
- **Graph builder generik pertama** ditambahkan di `src/domain/graph-builder.ts` (belum disambungkan ke UI/store — dites lewat `scripts/test-graph-builder.ts`). Pipeline: `resolveUltgScope()` membaca `sld-source-index.json` (17 folder GI/GIS ULTG Durikosambi) sebagai scope, `buildAnchorTopology()` memfilter `digsilentLineDb` (610 substation se-Jawa-Bali) ke substation dalam scope itu dan membangun `Substation`/`Bay`/`LineRelation` dari field eksplisit (`fromSubstation`/`toSubstation`/`fromTerminal`/`toTerminal`/impedance) — tanpa infer dari string bay, `overlaySettingDocs()` menempelkan detail relay/CT/VT dari LCD+DIST dan OCR registry ke bay yang sudah ada di anchor (overlay, bukan pembentuk topology baru), lalu `buildGraphForUltg()` mengelompokkan semuanya jadi satu `GraphBuildGroup` per substation — satu unit untuk direview, bukan satu per baris.
  ...
  - **Alias table eksplisit (`DIGSILENT_TO_SLD_ALIAS`)** ditambahkan untuk 3 kasus yang dikonfirmasi manual oleh engineer (bukan tebakan algoritma — disambiguasi by design harus bersumber dari konfirmasi lapangan, karena tebakan yang salah berisiko menggabungkan dua GI fisik berbeda menjadi satu):
    ...
    - Site pasca-migrasi yang tidak punya anchor 2021 sama sekali (**GI Muarakarang Baru** yang sebenarnya, GI Dadap, GIS Ulujami — total 3 dari 17) tetap `needsManualTopology: true`. Ini bukan bug — Dadap/Ulujami memang energize pasca-2022, dan `GI Muarakarang Baru` (site ketiga hasil migrasi, cuma punya 2 line ke GI Muarakarang Lama + GIS Muarakarang Baru) memang tidak direkam sebagai entitas terpisah di workbook 2021.
    - **2026-07-31 — koreksi**: `GISTET Durikosambi` dan `GISTET Kembangan` sebelumnya juga masuk daftar `needsManualTopology: true` di atas, tapi itu bug matching, bukan data yang benar-benar hilang dari workbook 2021 — `digsilentLineDb` sebenarnya sudah punya jalur 500kV lengkap (`DKSBI-KMBGN1/2`, `GNDUL-KMBGN-1/2`), hanya saja DIgSILENT menamai sisi 500kV suatu GI dengan suffix angka pada shortcode-nya (`DKSBI7`, `KEMBANGAN7`, `GANDUL7`) yang tidak match token apa pun terhadap scope SLD-nya. Diperbaiki di `graph-builder.ts` (`resolveScopedForRecord`, `parseLineVoltageKv`) — lihat `IMPLEMENTATION_NOTES.md`. GISTET Durikosambi dan GISTET Kembangan sekarang ter-anchor dengan benar sebagai 500kV, terpisah dari GI/GIS 150kV di lokasi fisik yang sama.
...
- **Coverage engine (`src/lib/corridor-math.ts`, `src/lib/coordination-checks.ts`) masih 1D linear murni** (`Corridor.ordered_segment_ids`, model `domain/types.ts` yang terpisah dari `unified.ts`) — secara struktural belum bisa menghitung Z2/Z3 dengan mempertimbangkan percabangan (`RemoteBusBranch`) sama sekali. `seed-corridor.ts` sekarang sudah dinamis untuk urutan node/label/start (lihat poin Study seed di atas), jadi *input*-nya sudah benar, tapi *perhitungan* Z1/Z2/Z3 itu sendiri masih 1D dan tidak bisa menembus ke trafo/percabangan.
  - **2026-07-31 — fondasi tipe (lapis 1 dari migrasi ini) sudah ada**: `RelaySetting` (plus `DistanceZoneSetting`, `LoadEncroachmentSetting`) ditambahkan ke `unified.ts`, field opsional `relaySettings?` di `UnifiedNetwork` — ini rumah bertipe untuk Z1/Z2/Z3 reach/timer terikat ke `RelayIED.id`, yang sebelumnya tidak ada sama sekali di model graph baru (`RelayIED` tanpa field zone, `SettingRecord.values` untyped bag).
  - **2026-07-31 — lapis 2 (algoritma graph-aware) sudah ada di `src/lib/graph-coordination.ts`**: `walkGraphReach` menelusuri reach sebagai akumulasi impedansi (ohm), bukan posisi km linear; di setiap remote bus, `selectGoverningBranch` memilih `RemoteBusBranch` dengan impedansi terbesar (line lanjutan vs trafo), sesuai prinsip zone-3 setting practice, lalu lanjut menembus (multi-hop) sampai reach habis atau data topologi habis. `runGraphCoordinationChecks` mem-port kelima check dari `coordination-checks.ts` (Z1/Z2 under/over-reach, Z2 timer sanity, Z2/Z3 margin, backup-gap + time-race lintas relay). Diverifikasi lewat `npm run test:graph-coordination` dengan fixture bercabang nyata (satu line dengan 2 kandidat cabang, trafo lebih besar dari line lanjutan).
  - **2026-07-31 — lapis 3 (migrasi consumer) selesai**: `CoverageView.tsx` dan `VerifiedReportView.tsx` sekarang memanggil `runGraphCoordinationChecks` atas `UnifiedNetwork`, bukan `runCoordinationChecks` atas `Topology`/`Corridor` lama. `CorridorDiagram.tsx` (visual d3, sumbu-x km linear) sengaja TIDAK dimigrasikan — masih dipakai apa adanya dengan label eksplisit di UI bahwa diagram visual belum graph-aware sementara diagnostics di bawahnya sudah. Builder baru `relay-catalog-builder.ts` (`buildRelayIedsFromCatalog`, dari 45 asset relay catalog berstatus `matched`) dan `relay-setting-builder.ts`'s `buildRelaySettingsFromOverlays` mengisi `relayIeds`/`relaySettings` untuk demo seed dari data nyata (10 RelayIED + 10 RelaySetting dengan Z1/Z2/Z3 dari dokumen TAP resmi, bukan lagi selalu kosong). `ZoneParameterPanel.tsx`/`RXPlaneModal.tsx` dan gate stage `coordination` di `setting-case.ts` masih belum disentuh — lihat `IMPLEMENTATION_NOTES.md` untuk detail dan alasan scope.
- `generateCorridorNmm()`/`nmm-generator.ts` sudah dihapus (2026-07-31) — dead code terkonfirmasi lewat audit, nol call site di manapun.
